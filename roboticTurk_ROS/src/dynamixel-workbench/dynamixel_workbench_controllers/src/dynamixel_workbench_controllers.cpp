...
bool DynamixelController::loadDynamixels(void)
{
  ...
  //## Add information to dynamixel driver as well
  uint8_t scanned_id[5] = {
      0,
  };
  uint8_t dxl_cnt = 0;
  // uint8_t range = 5;
  dxl_wb_->scan(scanned_id, &dxl_cnt, dynamixel_.size());
...
bool DynamixelController::getPresentPosition(std::vector<std::string> dxl_name)
{
  ...
  if (dxl_wb_->getProtocolVersion() == 2.0f)
  {
    ...
    for (uint8_t &id : id_array)
    {

      result = dxl_wb_->addBulkReadParam(id, "Present_Position", &log);
    ...
    result = dxl_wb_->bulkRead(&log);
    if (!result)
    {
      ROS_ERROR("%s", log);
    }

    result = dxl_wb_->getBulkReadData(get_position, &log);

    if (result == false)
    {
      ROS_ERROR("%s", log);
      result = dxl_wb_->clearBulkReadParam();
    }
    else
    {
      result = dxl_wb_->clearBulkReadParam();
    ...
    else
    {
      for (uint8_t index = 0; index < id_cnt; index++)
      {
        ...
        ROS_INFO("Current position %d for id %d", get_position[index], index);
...
void DynamixelController::readCallback(const ros::TimerEvent &)
{
  ...
  if (is_moving_ == false)
  {
    ...
    if (dxl_wb_->getProtocolVersion() == 2.0f)
    {
      ...
      ///### CUSTOM CONTROLLER - Use bulk read instead of sync read

      dxl_wb_->initBulkRead(&log);

      //#### Read Present Voltage

      for (uint8_t &id : id_array)
      {
        if (strcmp(dxl_wb_->getModelName(id), "XL-320") == 0)
        {
          result = dxl_wb_->addBulkReadParam(id, "Present_Voltage", &log);
        }
        else
        {
          result = dxl_wb_->addBulkReadParam(id, "Present_Input_Voltage", &log);
        }
        if (!result)
        {
          ROS_ERROR("%s", log);
      ...
      result = dxl_wb_->getBulkReadData(get_current, &log);
      ...
      for (uint8_t &id : id_array)
      {
        if (strcmp(dxl_wb_->getModelName(id), "XL-320") == 0)
        {
          result = dxl_wb_->addBulkReadParam(id, "Moving_Speed", &log);
        }
        else
        {
          result = dxl_wb_->addBulkReadParam(id, "Present_Velocity", &log);
        }
        if (!result)
        {
          ROS_ERROR("%s", log);
      ...
      result = dxl_wb_->getBulkReadData(get_velocity, &log);
...
void DynamixelController::writeCallback(const ros::TimerEvent &)
{
  ...
  uint8_t id_array[dynamixel_.size()];
  uint8_t id_array_i[dynamixel_.size()];

  uint8_t id_cnt = 0;

  int32_t dynamixel_position[dynamixel_.size()];
  int32_t dynamixel_position_i[dynamixel_.size()];
...
// Here are two methods for trying to make XL320 actuator to work with XL430 actuators, as it is one of them
// We are talking about "trying" because, servos starts to lag, when there are 5 or more connected
//These methods add more data to connection, so there is bottleneck somewhere
//TODO fix me?

//####### 1 ###########//
//First one uses syncWrite for five XL430 actuators and single goalPosition command for XL320 to move it
    ...
    //   if (strcmp(dxl_wb_->getModelName(id_array[i]), "XL-320") == 0)
    //   {
    //     result = dxl_wb_->goalPosition(id_array[i], dynamixel_position[i], &log);
    //     if (result == false)
    //     {
    //       ROS_ERROR("%s", log);
    //     }
    ...
    //    // dynamixel_position = std::remove(std::begin(dynamixel_position), std::end(dynamixel_position), dynamixel_position[i]);
    //   }else {
    //     id_array_i [id_cnt_i] = id_array[i];
    //     dynamixel_position_i[id_cnt_i] = dynamixel_position[i];
    //     id_cnt_i++;
    //   }
    // }


    // result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, id_array_i, id_cnt_i, dynamixel_position_i, 1, &log);
    // if (result == false)
    // {
    //   ROS_ERROR("%s", log);
    // }
...
//########## 2 ################//
// Second method is for using bulkWrite to send data as bulk for spefic addresses for each actuatuor, with single packet
// Something is here bottleneck as well, since when 5 or more actuators are connected, servos start to lag

    // dxl_wb_->initBulkWrite(&log);

    // for (uint8_t i = 0; i < id_cnt; i++)
    // {
    //   ROS_INFO("Moving id %d with position %d", id_array[i], dynamixel_position[i]);
    //   result = dxl_wb_->addBulkWriteParam(id_array[i], "Goal_Position", dynamixel_position[i], &log);
    ...
    //   }
    //   // else{
    //   //   ROS_INFO("%s for ID %d", log, id_array[i]);
    //   // }
    // }
...
// Just id based lookup for actuator, nonrelated probably
    //   for (uint8_t i = 0; i < id_cnt; i++)
    // {
    //  // ROS_INFO("%d", i);
    //   if (id_array[i] == 6) {
    //     //ROS_INFO("THERE IS MATCH");
    ...
    // DEFAULT method here, uses syncWrite for all connected actuators
